    ...
    with open(gpu_event_file_path, 'rb') as f:
        ...
        while raw_content_idx < raw_content_size:
            ...
            if npkit_event_def['id_to_type'][parsed_gpu_event['id']] == 'NPKIT_EVENT_TIME_SYNC_CPU':
                ...
                curr_cpu_base_time = 0
                print(f"cpu_clock_scale: {cpu_clock_scale}, timestamp: {parsed_gpu_event['timestamp']}, curr_cpu_base_time: {curr_cpu_base_time}, rank: {rank}, buf_idx: {buf_idx}")
                curr_gpu_base_time = None
                ...
                if curr_gpu_base_time is None:
                    ...
                    print(f"gpu_clock_scale: {gpu_clock_scale}, timestamp: {parsed_gpu_event['timestamp']}, curr_gpu_base_time: {curr_gpu_base_time}, rank: {rank}, buf_idx: {buf_idx}")
            else:
                if curr_gpu_base_time is None:
                    curr_gpu_base_time = parsed_gpu_event['timestamp'] / gpu_clock_scale
                ...
                if phase == 'B':
                    ...
                    gpu_events[-1].update({
                        ...
                        'args': {
                            ...
                            'buf_idx': buf_idx,
                            'seq': event_type_to_seq[event_type],
                            'rsvd_0': parsed_gpu_event['rsvd'] * 1000 // gpu_clock_scale,  ## ns
                            'size_0': parsed_gpu_event['size']
                        }
                    })
                    event_type_to_seq[event_type] += 1
    ...
    with open(cpu_event_file_path, 'rb') as f:
        ...
        while raw_content_idx < raw_content_size:
            ...
            if phase == 'B':
                ...
                # cpu_events[-1]['args']['bw (GB/s)'] = 0. if delta_time == 0. else cpu_events[-1]['args']['size'] / delta_time  ## GB/S = B/ns
