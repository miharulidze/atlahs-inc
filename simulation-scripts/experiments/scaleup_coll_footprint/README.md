...
## Two topologies, group size swept via the trace
...
```bash
docker run --rm -v "$(pwd)":/workspace atlahs-sim build            # one-time (ports the PT6 counter)
docker run --rm -v "$(pwd)":/workspace atlahs-sim run scaleup_coll_footprint --validate   # no-sim gen+compile check
docker run --rm -v "$(pwd)":/workspace atlahs-sim run scaleup_coll_footprint               # full sweep -> results CSV
...
## Outputs (`results/scaleup_coll_footprint/`, gitignored)
