...
def run_one_sim(binp, su_topo, so_topo, gpn, groups, mbps, pfc, timeout):
    ...
    pfc = the sim.pfc_config(su_topo) triple (high, low, egress_cap). Returns
...
def run_exp(speeds_gbps, layers, iters, tmpdir, timeout, do_plot, internode_gbps):
    ...
    # TODO guardrail (dev): only rates that are exact integer ps/B on the wire.
    bad = [g for g in speeds_gbps if 8000 % g]
    if bad:
        sys.exit(f"non-exact link speed(s) {bad} Gbps: htsim stores link rate as "
                 f"integer ps/B (= 8000/Gbps), so swept rates must divide 8000 "
                 f"(e.g. {DEFAULT_SPEEDS_GBPS})")
